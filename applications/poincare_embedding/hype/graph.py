...
sys.path.append('/home/canal/HYPER-LIB/Hyperbolic_Library')
from MCTensor import MCModule, MCEmbedding  # noqa
...
class MCHypEmbedding(MCModule):
    def __init__(self, size, dim, manifold, nc=2, sparse=True, com_n=1,
                 dtype=th.float64, device=th.device("cpu")):
        super(MCHypEmbedding, self).__init__()
        self.dim = dim
        self.nobjects = size
        self.manifold = manifold
        self.lt = MCEmbedding(
            size, com_n*dim, nc, sparse=sparse, dtype=dtype, device=device)
        print(">>>>>> The size of MCHypEmbedding:", self.lt.extra_repr())
    ...
    # init weight might be problem for MCEmb
    ...
    def forward(self, inputs):
        ...
        # not used in Halfspace and poincare
